/*
  ...
  1. Changes
    ...
    - Recreate function with proper role casting and improved error handling
*/

-- Drop existing function if exists
DROP FUNCTION IF EXISTS create_user_and_employee;

-- Recreate function with proper role casting and validation
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    AND enum_range(NULL::user_role)::text[] @> ARRAY[p_role]
  ) INTO v_valid_role;

  IF NOT v_valid_role THEN
    RAISE EXCEPTION 'Invalid role: %. Must be one of: %', p_role, enum_range(NULL::user_role)::text[];
  END IF;
  ...
  -- Create employee_departments entry if department is specified
